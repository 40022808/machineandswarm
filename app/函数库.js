import {
    ...
    文本2_后续2_回答,文本2_后续3,文本2_后续3_回答,文本2_后续2_效果, 文本2_后续1_奖励, 文本2_后续2_奖励, 文本3_奖励,文本2_后续3_效果
} from "./文本.js";
    ...
    function 创造新技能(arr) {
        arr.forEach(id => {
            if (!document.getElementById(id)) {
                ...
                if (是否有技能 == 'no') {
                    newElement.className = '技能按钮 显示'
                    是否有技能 = 'yes'
                }
                else {
                    newElement.className = '技能按钮'
                }
                document.querySelector('.战斗_技能_库').appendChild(newElement);
...
function 个人信息_名字() {
    ...
    const 内容_上半部分_人物h1 = document.querySelector('.内容_上半部分_人物h1')
    名字显示.innerHTML += '[' + name_code.value + ']'
    内容_上半部分_人物h1.innerHTML = '[' + name_code.value + ']'
}
...
function 奖励弹窗_关闭_显示() {
    ...
    奖励弹窗_关闭.style.display = 'block'
}

function 奖励弹窗_关闭_消失() {
    const 奖励弹窗_关闭 = document.querySelector('.奖励弹窗_关闭')
    奖励弹窗_关闭.style.display = 'none'
}
...
function 技能_顺风斩() {
    ...
    function 处理NPC(npcSelector, 防御力Selector, 生命Selector) {
        ...
        if (生命.textContent !== '') {
            ...
            let 攻击力 = Math.round(medate.me攻击力 * (0.8 + ((meskill.顺风斩  - 1) / 10)))
            let 伤害 = 攻击力 >= 防御力 ? 攻击力 - 防御力 : 0;
